...
from predata.datapre import imdbkerasdata
import time
from net.RNNet import IMBDKerasNet

args = utils.parse_args()
MAX_WORDS = args.max_words  # imdb’s vocab_size 即词汇表大小
MAX_LEN = args.seq_len      # max length
...
data = imdbkerasdata(batchsize=BATCH_SIZE,seq_len=args.seq_len,out_dim=args.out_dim)
...
model = IMBDKerasNet(MAX_WORDS, EMB_SIZE, HID_SIZE, DROPOUT).to(DEVICE)
print(model)
optimizer = optim.Adam(model.parameters())

PATH = "model/imdbkeras_em" + str(args.embedding_dim) + "train" + str(args.epochs) + "hidden" + str(
